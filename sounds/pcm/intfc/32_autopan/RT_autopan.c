...
//************************************************************************//
    ...
    waveOutOpen(&out_hdl, 0, &wave_format_ex_out, 0, 0, CALLBACK_NULL); // 出力(再生)デバイスオープン
    ...
    while (1){
        ...
        if (flag == 1){                                             // flag=1なら出力状態
            if ((out_hdr[out1].dwFlags & WHDR_DONE) != 0){          // 出力バッファのフラグ情報が0でないなら処理を実行
                for (n = 0; n < BUF_SIZE/2; n++){                   // 出力をBUF_SIZEサンプル分だけ書き出す
                    to=(to+1)%MEM_SIZE;                             // 出力時刻の更新
                    out_buf[out1][2*n]   = y_L[to]*32767;           // バッファにLch出力データ書き込み
                    out_buf[out1][2*n+1] = y_R[to]*32767;           // バッファにRch出力データ書き込み
                }
                waveOutWrite(out_hdl, &out_hdr[out1], sizeof(WAVEHDR));// 出力待ちキューに出力バッファデータを送信
                out1=(out1+1)%NUM_BUF;                              // 出力バッファ番号を更新
                for (n = 0; n < BUF_SIZE/2; n++){                   // 入力の2倍出力するため再度BUF_SIZEサンプル分だけ書き出す
                    to=(to+1)%MEM_SIZE;                             // 出力時刻の更新
                    out_buf[out1][2*n]   = y_L[to]*32767;           // バッファにLch出力データ書き込み
                    out_buf[out1][2*n+1] = y_R[to]*32767;           // バッファにRch出力データ書き込み
                }
        ...
        if ((in_hdr[in1].dwFlags & WHDR_DONE) != 0){                // 入力バッファのフラグ情報が0でないなら処理を実行
            ...
            for (n = 0; n < BUF_SIZE; n++){                         // BUF_SIZEサンプル分の入力を読込み，処理する
                ...
                y_L[t] = s[t];                                      // 出力＝入力の素通しをつくる
                y_R[t] = s[t];                                      // 出力＝入力の素通しをつくる
...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    a = 1.0 + sin(2.0*M_PI*r*l/(double)Fs);         // 正弦波を更新
                    y_L[t] = a*s[t];                                // 正弦波を入力に乗じたものをLch出力

                    a = 1.0 - sin(2.0*M_PI*r*l/(double)Fs);         // 負の正弦波を更新
                    y_R[t] = a*s[t];                                // 正弦波を入力に乗じたものをRch出力
