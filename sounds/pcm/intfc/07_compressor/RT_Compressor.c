...
//************************************************************************//
    ...
    double Th;                                                      // しきい値
    double ratio;                                                   // しきい値以上の振幅の増幅率
    double gain;                                                    // 処理後出力の最大値調整用
    ///////////////////////////////
    //                           //
    //      変数の初期設定       //
    //                           //
    ///////////////////////////////
    Th    = 0.2;                                                    // しきい値
    ratio = 0.1;                                                    // しきい値以上の振幅の増幅率（1以下）
    gain  = 1.0/(Th+(1.0-Th)*ratio);                                // 処理後出力の最大値調整用(最大値0.7)
...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    if(s[t]>Th)        s[t]= Th+(s[t]-Th)*ratio;        // 大振幅の圧縮
                    else if (s[t]<-Th) s[t]=-Th+(s[t]+Th)*ratio;        // 大振幅の圧縮
                    s[t] = s[t]*gain;                                   // 最終の振幅調整
                    y[t] = s[t];
