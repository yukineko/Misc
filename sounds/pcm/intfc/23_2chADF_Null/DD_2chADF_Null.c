...
//************************************************************************//
        ...
        x=0,norm=0;
        for(i=0;i<=N;i++){
            x=x+s_L[(t-D-i+MEM_SIZE)%MEM_SIZE]*h[i];        // Lch信号をフィルタリング
            norm=norm+s_L[(t-D-i+MEM_SIZE)%MEM_SIZE]*s_L[(t-D-i+MEM_SIZE)%MEM_SIZE];
        }
        e = s_R[t] - x;                                     // 推定誤差
        if(norm>0.00001){                                   // ノルムが一定値以上で係数更新
            for(i=0;i<=N;i++){                              // NLMSアルゴリズム
                h[i] = h[i] + mu * s_L[(t-D-i+MEM_SIZE)%MEM_SIZE] * e/norm;
            }
        }
        y[t]=e;                                             // 推定誤差を出力とする
