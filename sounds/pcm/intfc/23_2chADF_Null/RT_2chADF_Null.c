...
//************************************************************************//
    ...
    long int l=0;                                                   // 2ch時刻管理用
    int      D;                                                     // Lch ADF遅延量
    double   s_L[MEM_SIZE+1]={0}, s_R[MEM_SIZE+1]={0};              // 入力データ格納用
    double   x, e;                                                  // 観測信号, 誤差信号
    double   mu, norm, h[N+1];                                      // ステップサイズ、ノルム、フィルタ係数
...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    if(t%2==1){
                        ...
                        s_R[l] = s[t];                              // Rch入力
                        x=0, norm=0;
                        for(i=0;i<=N;i++){                          // Lch信号をフィルタリング
                            x=x+s_L[(l-D-i+MEM_SIZE)%MEM_SIZE]*h[i];
                            norm=norm+s_L[(l-D-i+MEM_SIZE)%MEM_SIZE]*s_L[(l-D-i+MEM_SIZE)%MEM_SIZE];
                        }
                        e = s_R[l] - x;                             // 推定誤差
                        if(norm>0.00001){                           // ノルムが一定値以上で係数更新
                            for(i=0;i<=N;i++){                      // NLMSアルゴリズム
                                h[i] = h[i] + mu * s_L[(l-D-i+MEM_SIZE)%MEM_SIZE] * e/norm;
                            }
                        }
                        l=(l+1)%MEM_SIZE;                           // 現在時刻を進める
                    }
                    y[t]= e;                                        // 推定誤差を出力とする
