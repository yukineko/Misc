...
//************************************************************************//
        ...
        x=0,norm=0;
        for(i=0;i<=N;i++){
            x=x+y[(t-D-i+MEM_SIZE)%MEM_SIZE]*h[i];          // 過去の出力信号から現在の入力を推定
            norm=norm+y[(t-D-i+MEM_SIZE)%MEM_SIZE]*y[(t-D-i+MEM_SIZE)%MEM_SIZE];// ノルムの計算
        }
        e = s[t] + 0.5*y[(t-D-i+MEM_SIZE)%MEM_SIZE] - x;                                       // 推定誤差
        if(norm>0.00001){                                   // ノルムが一定値以上ならフィルタ係数を更新
            for(i=0;i<=N;i++){                              // NLMSアルゴリズム
                h[i] = h[i] + mu * y[(t-D-i+MEM_SIZE)%MEM_SIZE] * e/norm;
            }
        }
        y[t] = e;                                           // 推定誤差を出力とする
