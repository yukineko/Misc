...
//************************************************************************//
        ...
        for (i=0;i<OL;i++){
            ...
            y[t]=y[t]+z1[i][nm[i]]/FFT_SIZE/(OL/2);         // 出力計算
        }
        ...
        if(l>=FFT_SIZE/OL){
            ...
            for(m=0;m<FFT_SIZE/2;m++){
                if(Xamp[m]<=0.000001)Xamp[m]=0.000001;      // 振幅の最小値を制限する
                spec = Xamp[m]*Xamp[m]*256;                 // パワースペクトル
//                spec = 10*log10(spec);                      // 対数パワースペクトル
                if(spec < 256){                             // clrY, Cr, Cb で 256×3=768の大きさを表現
                    Cb  = spec;                             // spec < 256 ならCbだけでspecを表現
                    Cr  = 0;
                    clrY= 0;
                }
                if( spec >= 256 && spec < 512 ){            // spec < 512 ならCbとCrでspecを表現
                    Cb  = 255;
                    Cr  = spec-255;
                    clrY= 0;
                }
                if( spec >= 512 ){                          // spec >=512 なら Cb, Cr, clrYでspecを表現
                    Cb  = 255;
                    Cr  = 255;
                    clrY= spec-511;
                    if(clrY > 255) clrY=255;
                }
