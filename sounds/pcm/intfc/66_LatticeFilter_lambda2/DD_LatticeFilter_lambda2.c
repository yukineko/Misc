...
//************************************************************************//
    ...
    int      i;                                             // forループ用変数
    double   f[N+1]={0}, b0[N+1]={0}, b1[N+1]={0};          // 前向き予測誤差，後ろ向き予測誤差
    double   rf[N+1]={0},rb[N+1]={0};                       // 反射係数
    double   k[N+1]={0},Eb0[N+1]={0}, Eb1[N+1]={0}, Ef[N+1]={0}; // 係数更新用変数
    double   ramda, ramda2;                                 // ラティス忘却係数
    ...
    ramda  = 1.0 - (1.0 / N);                               // 忘却係数の設定
    ramda2 = 1.0 - (0.1 / N);                               // rambda <= rambda2
...
//************************************************************************//
        ...
        ////////////////////////////////////////////////////
         ...
         f[0] =  b0[0] = s[t];                              // ラティスフィルタへの入力信号
        Ef[0] = Eb0[0] = ramda2 * Ef[0] + (s[t] * s[t]);    // 入力信号の分散の推定
        for (i =1 ; i <= N; i++) {                          // 出力計算ループ
            k[i-1]   = ramda  *   k[i-1] +  f[i-1] *  b1[i-1];// 反射係数の分子
            if (Eb1[i-1] != 0.0) rf[i] = -k[i-1] / Eb1[i-1];// 前向き反射係数の更新
            if ( Ef[i-1] != 0.0) rb[i] = -k[i-1] /  Ef[i-1];// 後向き反射係数の更新
              f[i]   = f[i-1]  + rf[i] * b1[i-1];           // 前向き予測誤差
             b0[i]   = b1[i-1] + rb[i] *  f[i-1];           // 後向き予測誤差
             Ef[i]   = ramda2 *  Ef[i]   +  f[i] *  f[i];   // 前向き反射係数の分母更新
            Eb0[i]   = ramda2 * Eb0[i]   + b0[i] * b0[i];   // 後向き反射係数の分母更新
