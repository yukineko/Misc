...
//************************************************************************//
    ...
    double p[FFT_SIZE+1]={0},d1[FFT_SIZE+1]={0},d2[FFT_SIZE+1]={0}; //ピーク検出用変数
    double h1[6]={0},h2[6]={0};                             // 微分フィルタ係数
    int    k,m;                                             // 周波数番号の変数
...
//************************************************************************//
        ...
        if( l>=FFT_SIZE/2){                                 // 半フレームごとにFFTを実行
            ...
            for(k=1;k<FFT_SIZE/2;k++){
                p[FFT_SIZE-k]=p[k];                         // ピークスペクトルを対象に配置
            }
            for(i=0;i<FFT_SIZE;i++){
                if(p[i]==0){
                    Xr[i]=0;                                // 実部の処理
