...
//************************************************************************//
    ...
    r=0.8;                                                  // 極半径の２乗
    fc=1000.0/Fs;                                           // 位相反転周波数
    a=-(1+r)*cos(2.0*M_PI*fc);                              // フィルタ係数
...
//************************************************************************//
        ...
        u0  = s[t]   - a * u1 - r * u2;                     // 内部信号生成
        x   = r * u0 + a * u1 + u2;                         // オールパスフィルタ出力
        u2  = u1;                                           // 信号遅延
        u1  = u0;                                           // 信号遅延

        fc  = fc + 0.0005*Fs;                               // 反転する周波数の変更
        if(fc>Fs) fc=0;                                     // 最大周波数で0に戻す
        a   = -(1+r)*cos(2.0*M_PI*fc/Fs);                   // フィルタ係数
