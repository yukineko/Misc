...
//************************************************************************//
        ...
        p   =  p + r;                                       // 設定した r で出力時刻 p を決める
        if(p>=MEM_SIZE)p=p-MEM_SIZE;                        // MEM_SIZEを超えないようにする
        j=(int)p;                                           // pの整数部を取得
        ln=p-j;                                             // pの小数部を取得
        y[t]= (1.0-ln) * s[j] + ln * s[(j+1)%MEM_SIZE];     // 線形補間により振幅値を決める
