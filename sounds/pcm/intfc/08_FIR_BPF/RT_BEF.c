...
//************************************************************************//
    ...
    //                           //
    ///////////////////////////////
    fe1=1000.0;                                                     // 低域側遮断周波数[Hz]
    fe2=3000.0;                                                     // 高域側遮断周波数[Hz]
    ...
    for(i=-N/2;i<=N/2;i++){                                         // 係数の設定
        if(i==0) h[N/2+i]=1-2.0*fe2+2.0*fe1;
        else{
            h[N/2+i]=sin(M_PI*i)/(M_PI*i)-2.0*fe2*sin(2.0*M_PI*fe2*i)/(2.0*M_PI*fe2*i)+2.0*fe1*sin(2.0*M_PI*fe1*i)/(2.0*M_PI*fe1*i);
        }
