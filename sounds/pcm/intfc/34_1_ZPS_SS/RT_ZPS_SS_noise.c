...
//************************************************************************//
    ...
    double   d0[FFT_SIZE+1]={0};                                    // 推定ノイズZPS
    int      NM, cnt, L;                                            // ノイズ推定フレーム数
    double   d;                                                     // ノイズ
...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    if( l>=FFT_SIZE/2){                             // 半フレームごとにFFTを実行
                        ...
                        ////////////////////////////////////////////////////
                        if(cnt<NM){
                            for(i=0;i<=FFT_SIZE/2;i++){             // ノイズ推定
                                d0[i]=d0[i]+s0[i]/(double)NM;       // 平均値を計算
                                d0[FFT_SIZE-i]=d0[i];               // 対称性の確保
                            }
                            cnt++;
                        }

                        for(i=0;i<FFT_SIZE;i++){
                            s0[i]=s0[i]-d0[i];                      // ノイズ推定値を減算
                        }
                        izpt();                                     // 逆ゼロ位相変換
                        l=0;
                    }
