...
//************************************************************************//
    ...
    printf("[Enter]                --> End\n");                                     // [Enter]で終了
    printf("[number key]           --> Band Quality Up\n");               // 数字キーで感度切り替え
    printf("[Shift] + [number key] --> Band Quality Down\n");               // 数字キーで感度切り替え
    printf("Through\n");                                            // 素通しであることを表示
...
//************************************************************************//
        ...
        if (kbhit()){                                               // キーボード入力があったか
            ...
            //                                          //
            //////////////////////////////////////////////
            if ( key>=49 && key<=49+N-1){                           // 数字キーが押されとき
                Q[key-48]=Q[key-48]+1.0;                            // クオリティファクタを0.1増加
                if(Q[key-48]>10)Q[key-48]=10;                     // 1を超えると-1に変更
                printf("Lv ");
                for(i=1;i<=N;i++){
                    printf("%d, ",(int)Q[i]);
                }
            ...
            if ( key>=33 && key<=33+N-1){                           // 数字キーが押されとき
                Q[key-32]=Q[key-32]-1.0;                            // クオリティファクタを0.1増加
                if(Q[key-32]<0)Q[key-32]=0;                     // 1を超えると-1に変更
                printf("Lv ");
                for(i=1;i<=N;i++){
                    printf("%d, ",(int)Q[i]);
                }
