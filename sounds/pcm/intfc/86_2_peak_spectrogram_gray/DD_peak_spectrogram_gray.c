...
//  Savitzky and Golay フィルタによるピーク検出       //
//                                                    //
...
int main(int argc, char **argv){
    ...
    double p[FFT_SIZE+1]={0},d1[FFT_SIZE+1]={0},d2[FFT_SIZE+1]={0};
    double h1[6]={0},h2[6]={0};                             // S-Gフィルタ係数
...
//************************************************************************//
    ...
    height          = FFT_SIZE/2;                           // 画像の高さFFTサイズ）
    width           = len*OL/FFT_SIZE;                      // 画像の幅 (フレーム長)
...
//************************************************************************//
    ...
    // S-Gフィルタの係数設定
    h1[0]=-2/10.0, h1[1]=-1/10.0, h1[2]=0,      h1[3]=1/10.0, h1[4]=2/10.0;
    h2[0]= 2/7.0,  h2[1]=-1/7.0,  h2[2]=-2/7.0, h2[3]=-1/7.0, h2[4]=2/7.0;

    n=0;                                                    // 画像の横座標(フレーム番号に対応)
...
//************************************************************************//
        ...
        if(l>=FFT_SIZE/OL){
            ...
            for(k=0;k<FFT_SIZE;k++){
                Xamp[k]=log(Xamp[k]);                       // 対数振幅スペクトルに変換
            }
            //FL=5;             フィルタ係数の数
            for(k=4;k<FFT_SIZE;k++){                        // 1次微分フィルタ
                d1[k-2]=0;
                for(m=0;m<5;m++){
                    d1[k-2]=d1[k-2]+h1[m]*Xamp[k-m];
                }
            }
            for(k=4;k<FFT_SIZE;k++){                        // 2次微分フィルタ
                d2[k-2]=0;
                for(m=0;m<5;m++){
                    d2[k-2]=d2[k-2]+h2[m]*Xamp[k-m];
                }
            }
            for(k=1;k<=FFT_SIZE/2;k++){                     // ピーク検出フィルタ
                p[k]=0;
                if(d1[k]*d1[k-1]<0){                        // 1階微分の隣接サンプルの符号が異なる場合
                    if(fabs(d1[k])<fabs(d1[k-1])){
                        if(d2[k]  <0) p[k]=1;               // 2階微分が負ならピーク
                    }
                    else{
                        if(d2[k-1]<0) p[k-1]=1;             // 2階微分が負ならピーク
                    }
                }
            }

            for(m=0;m<FFT_SIZE/2;m++){
                if(Xamp[m]<0)Xamp[m]=0;
                Bout=Gout=Rout= p[m] * Xamp[m]*256;
                DataWritePoint = FileHeaderSize + ( m*(int)width + n )*3;// 3は色の数
