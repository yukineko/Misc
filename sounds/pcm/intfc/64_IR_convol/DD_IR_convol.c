...
//************************************************************************//
    ...
    //                                  //
    //////////////////////////////////////
    if( argc != 3 ){                                        // 使用方法が間違った場合の警告
        fprintf( stderr, "Usage: ./a speech.wav impulse_response.wav\n" );
        exit(-1);
    }
    ...
    if( (f0 = fopen(argv[2], "rb")) == NULL ){              // 出力ファイルが開けない場合の警告
        fprintf( stderr, "Cannot open %s\n", argv[2] );
        exit(-2);
    }
...
//************************************************************************//
    ...
    fseek(f0, 44L, SEEK_SET);                               // 音声データ開始位置に移動
    h_norm=0;
    for(i=0;i<N;i++){                                       
        if(fread( &input, sizeof(short), 1,f0) < 1) break; // インパルス応答の読み込み
        h[i]=input/32768.0;                                // インパルス応答をフィルタ係数にする
        h_norm=h_norm+h[i]*h[i];
    }
    L=i;
    for(i=0;i<L;i++){
        h[i]=h[i]/h_norm;                                   // 係数の正規化
    }
    printf("IR Length=%d\n",L);
...
//************************************************************************//
        ...
        x=0;
        for(i=0;i<L;i++){
            x=x+s[(t-i+MEM_SIZE)%MEM_SIZE]*h[i];            // インパルス応答を畳み込む
        }
        y[t] = x;                                           // フィルタ出力
