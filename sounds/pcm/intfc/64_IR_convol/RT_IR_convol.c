...
int main(int argc, char **argv){                                    // BGMファイルを指定させる方式
    //wav file読み込み用変数
    FILE *f1;                                                       // 指定されたファイルを表す変数
...
//************************************************************************//
    ...
    //////////////////////////////////////
    if( argc != 2 ){
        fprintf( stderr, "Usage: RT_BGM_plus_mic.exe BGM.wav \n" );
        exit(-1);
    }
...
//************************************************************************//
    ...
    h_norm=0;
    for(i=0;i<N;i++){                                       
        if(fread( &input, sizeof(short), 1,f1) < 1) break;          // インパルス応答の読み込み
