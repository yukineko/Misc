...
//************************************************************************//
    ...
    for(i=0;i<N;i++){                                               // 未知系の係数を乱数で設定
        g[i]=( (double)rand()/RAND_MAX*2-1.0 )*10/(double)N;           // 未知システムの係数を設定
    }
...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    sp  = s[t];                                     // 読み込んだ音声は外乱spにする
                    s[t]=(double)rand()/RAND_MAX*2-1.0;             // ノイズ源
                    x=0,d=0, norm=0;
                    for(i=0;i<N;i++){
...
//************************************************************************//
        ...
        if (kbhit()){                                               // キーボード入力があったか
            ...
            if (key == 32){                                         // スペースキーが押されとき
                ...
                if(hflg==1) printf("Noisy Signal\n");               // 観測信号であることを表示
                if(hflg==2) printf("ADF output\n");                 // 適応フィルタ出力であることを表示
            }
