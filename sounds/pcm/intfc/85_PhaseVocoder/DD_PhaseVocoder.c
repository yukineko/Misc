...
#include <string.h>
#include <math.h>
#define  MEM_SIZE 1600                                      // 音声メモリのサイズ
#define  FFT_SIZE 512                                       // FFT点数
#define  N        200                                       // 記憶するフレーム数
///////////////////////////////////////////////
//                                           //
//                                           //
//            FFT関数の構築                  //
...
void fft( void ){
    ...
    for(j=0;j<FFT_SIZE;j++){
        ...
        Xphs[j] = atan2(Fi[j], Fr[j]);                      // 位相スペクトルの算出
    }
}
...
//************************************************************************//
    ...
    double   Amp[N+1][FFT_SIZE/2+1]={0};                    // 振幅スペクトル保存用
    double   Phs[N+1][FFT_SIZE/2+1]={0};                    // 合成音声用の位相スペクトル管理
    double   Xphase[FFT_SIZE/2+1]={0};                      // 位相スペクトル
    int      outlen, fm;                                    // 出力データ長，入力フレーム番号
    double   fmout;                                         // 出力フレーム番号
    double   alpha;                                         // 音声の高さの倍率
...
//************************************************************************//
    ...
    //                                  //
    //////////////////////////////////////
    outlen = (int)(len/alpha);                              // 出力データの長さ調整
    Fs_out        = Fs;                                     // 出力サンプリング周波数を設定
    channel       = ch;                                     // 出力チャネル数を設定
    data_len      = channel*(outlen)*2;                     // 出力データの長さ = 全Byte数 (1サンプルで2Byte)
...
//************************************************************************//
        ...
        if( l>=FFT_SIZE/2){                                 // 半フレームごとにFFTを実行
            ...
            ///////////////////////////
            for(i=0;i<=FFT_SIZE/2;i++){
                Amp[fm][i]=Xamp[i];                         // 全振幅スペクトルの記録
                Phs[fm][i]=Xphs[i];                         // 全位相スペクトルの記録
            }
            l=0;
            fm=(fm+1);                                      // 入力フレーム番号
        }
    ...
    l=0;
    t=0;
    for(t_out=0;t_out<outlen;t_out++){                      // メインループ
        y[t] = (z[l]+z1[l])/FFT_SIZE;                       // IFFTで得た出力
        z1[l]= z[FFT_SIZE/2+l];                             // ハーフオーバーラップ用に出力記録

        if( l>=FFT_SIZE/2){                                 // 半フレームごとにFFTを実行
            fmout=fmout+alpha;                              // 出力フレーム番号の更新
            for(i=0;i<=FFT_SIZE/2;i++){
                Xphase[i]=Phs[(int)fmout][i];               // 位相はそのフレームのものを採用
                Xr[i]=Amp[(int)fmout][i]*cos(Xphase[i]);    // 合成信号の実部
                Xi[i]=Amp[(int)fmout][i]*sin(Xphase[i]);    // 合成信号の虚部
                Xr[FFT_SIZE-i]= Xr[i];                      // 実部は遇対称
                Xi[FFT_SIZE-i]=-Xi[i];                      // 虚部は奇対称
            }
            ifft();                                         // IFFT
            l=0;
        }
