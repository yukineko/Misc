...
//************************************************************************//
    ...
    int      D, i;                                          // 遅延量, ループ計算用変数
    double   x, e;                                          // 観測信号，推定誤差
    double   mu, norm;                                      // ステップサイズ，ノルム
...
//************************************************************************//
        ...
        x=0,norm=0;
        for(i=0;i<N;i++){
            x=x+s[(t-D-i+MEM_SIZE)%MEM_SIZE]*h[i];          // 予測値の計算
            norm=norm+s[(t-D-i+MEM_SIZE)%MEM_SIZE]*s[(t-D-i+MEM_SIZE)%MEM_SIZE];
        }
        e = s[t] - x;                                       // 現在の信号と予測値の誤差
        if(norm>0.00001){                                   // ノルムが一定値以上で係数更新
            for(i=0;i<N;i++){                               // NLMSアルゴリズムで係数更新
