...
//************************************************************************//
    ...
    double y_L[MEM_SIZE+1]={0}, y_R[MEM_SIZE+1]={0};                // 出力データ格納用変数
    ...
    while (1){
        ...
        if (flag == 1){                                             // flag=1なら出力状態
            if ((out_hdr[out1].dwFlags & WHDR_DONE) != 0){          // 出力バッファのフラグ情報が0でないなら処理を実行
                for (n = 0; n < BUF_SIZE; n++){                     // 出力をBUF_SIZEサンプル分だけ書き出す
                    ...
                    if(to%2==1){
                        out_buf[out1][n] = y_L[to]*32767;			// バッファに出力データ書き込み
                    }
                    else{
                        out_buf[out1][n] = y_R[to]*32767;			// バッファに出力データ書き込み
                    }
                }
        ...
        if ((in_hdr[in1].dwFlags & WHDR_DONE) != 0){                // 入力バッファのフラグ情報が0でないなら処理を実行
            ...
            for (n = 0; n < BUF_SIZE; n++){                         // BUF_SIZEサンプル分の入力を読込み，処理する
                ...
                if(t%2==1)y_L[t] = s[t];                            // 出力＝入力
                else      y_R[t] = s[t];                            // 出力＝入力
...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    if(t%2==1){
                        y_L[t]= s_L[(l-1+MEM_SIZE)%MEM_SIZE]-s_R[(l-1+MEM_SIZE)%MEM_SIZE];
                        s_L[l] = s[t];                              //Lch入力
                    }
                    else{
                        s_R[l] = s[t];                              //Rch入力
