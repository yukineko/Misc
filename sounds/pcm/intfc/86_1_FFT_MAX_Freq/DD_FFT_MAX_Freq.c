...
#include <string.h>
#include <math.h>
#define  MEM_SIZE 24000                                     // 音声信号用メモリサイズ
#define  FFT_SIZE 8192                                      //FFT点数
///////////////////////////////////////////////
//                                           //
//                                           //
//            FFT関数の構築                  //
...
//************************************************************************//
    ...
    double   pitch, max;                                    // 最大値
    int      max_pos;                                       // 最大パワーの周波数
...
//************************************************************************//
        ...
        if( l>=FFT_SIZE/2){                                 // 半フレームごとにFFTを実行
            ...
            for(i=0;i<FFT_SIZE/2;i++){
                if(max<Xamp[i]){
                    max=Xamp[i];                            // 最大振幅の更新
                    max_pos=i;                              // 最大周波数番号の更新
                }
            }
            pitch = Fs*(double)max_pos/FFT_SIZE;            // 最大周波数をHzに変換
            printf("pitch = %f[Hz]\n", pitch);              // 最大周波数を表示
            ifft();
            l=0;
        }
