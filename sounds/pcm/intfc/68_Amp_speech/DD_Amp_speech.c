...
//************************************************************************//
    ...
    double Xphs[FFT_SIZE+1]={0};                            // 位相スペクトル
...
//************************************************************************//
        ...
        if( l>=FFT_SIZE/2){                                 // 半フレームごとにFFTを実行
            ...
            ///////////////////////////
            for(i=0;i<FFT_SIZE/2;i++){
                Xphs[i] = 2*M_PI*(double)rand()/RAND_MAX;   // 位相は乱数
                Xphs[FFT_SIZE-i]=-Xphs[i];                  // 奇対称にする
            }
            for(i=0;i<FFT_SIZE;i++){
                Xr[i]=Xamp[i]*cos(2*M_PI*i/FFT_SIZE+Xphs[i]);//実部の処理
                Xi[i]=Xamp[i]*sin(2*M_PI*i/FFT_SIZE+Xphs[i]);//虚部の処理
            }
