...
//************************************************************************//
    ...
    while(1){                                               // メインループ
        if(fread( &input, sizeof(short), 1,f1) < 1)break;   // 音声を input に読込み
...
//************************************************************************//
    ...
    data_len      = channel*t_out*2;                        // 出力の全Byte数(1サンプルで2Byte)
    file_size     = 36+data_len;                            // 全体ファイルサイズ
    fseek(f2, 4L, SEEK_SET);                                // サンプル数情報位置に移動
    fwrite(&file_size,    sizeof(unsigned long ), 1, f2);   //書き込み
    fseek(f2, 40L, SEEK_SET);                               // サンプル数情報位置に移動
    fwrite(&data_len,     sizeof(unsigned long ), 1, f2);   //data_len
