...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    if(s[t]==0)s[t]=0.000001;                       // 0は除いておく
                    s[t] = (s[t] * pow(2,b-1));                     // 最大値倍する．最大値は正負とも2^{b-1}
                    if(hflg==1){                                    // 0を含めない量子化
                        y[t] = ceil( fabs(s[t]) );                  // 絶対値を切り上げて整数化
                        if(s[t]<0) y[t] = -y[t];                    // s[t]の符号に合わせる
                    }
                    if(hflg==2){                                    // 0を含める量子化
                        y[t] = floor( fabs(s[t])+0.5 );             // 絶対値を四捨五入して整数化
                        if( s[t]<0 ) y[t] = -y[t];                  // s[t]の符号に合わせる
                        if( y[t]>pow(2,b-1)-1 ) y[t]=pow(2,b-1)-1;  // 0を含めるため最大値を１つ下げる
                    }
                    y[t]=y[t]/pow(2.0, b-1);                        // 正規化
...
//                    y[t]=atan(y[t])/(M_PI/2.0);                         // クリップ防止
        ...
        if (kbhit()){                                               // キーボード入力があったか
            ...
            if (key == 32){                                         // スペースキーが押されとき
                ...
                if(hflg==1)printf("Quantization ( nonzero  )\n");   // 量子化0を含める
                if(hflg==2)printf("Quantization ( 0 exists )\n");   // 量子化0を含めない
            }
