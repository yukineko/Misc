...
//************************************************************************//
    ...
    double    phs[Fs+1]={0};                                                    // 初期位相
    double   window;                                                            // 窓関数
    double     z[Fs+1]={0}, z1[Fs+1]={0};                                       // オーバーラップ用出力
    int      k;                                                                 // 時刻の変数
...
//************************************************************************//
    ...
    data_len      = channel*(hi*2*wd/2)*2;                                        // 全Byte数(1サンプルで2Byte)
...
//************************************************************************//
    ...
    for(i=0;i<hi;i++){
        phs[i]=2*M_PI*(double)rand()/RAND_MAX;                                  // 初期位相を乱数で設定
    }
...
//                  Xamp[m] = 10*(exp( (B+G+R)/3.0/20.0 )-1);                     // スペクトログラムを自然対数とみなす
//                  Xamp[m] = 10*(pow( 10, (B+G+R)/3.0/20.0 )-1);                 // スペクトログラムを常用対数とみなす
            }
            for(i=0;i<2*hi;i++){                                                // 出力計算
                window = 0.5*(1.0-cos(2.0*M_PI*i/(2*hi)));                      // ハニング窓
                z[i]=0;
                for(k=0;k<hi;k++){
                    z[i]=z[i] + Xamp[k]/hi * cos(2.0*M_PI*k/(2.0*hi)*i+phs[k]) * window;
                }
            }
            t=0;
            n++;
         ...
         y[t] = z[t]+z1[t];                                                     // オーバーラップ出力
        z1[t] = z[hi+t];                                                        // オーバーラップ用信号の記録
