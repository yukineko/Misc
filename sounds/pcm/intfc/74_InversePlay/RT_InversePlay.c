        ...
        if (flag == 1){                                             // flag=1なら出力状態
            if ((out_hdr[out1].dwFlags & WHDR_DONE) != 0){          // 出力バッファのフラグ情報が0でないなら処理を実行
                for (n = 0; n < BUF_SIZE; n++){                     // 出力をBUF_SIZEサンプル分だけ書き出す
                    ...
                    out_buf[out1][n] = y[to]*32767;                 // 出力データを記録
                }
...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    if(Rec==1){
                        s[t] = in_buf[in1][n]/32768.0;              // 入力バッファからデータ読み込み
                    }
                    y[t] = s[t];                                    // 出力＝入力
                    ...
                    if(t==0){
                        Rec=-Rec;                                   // Recフラグ変更
                        if(Rec==1){
                            printf("Enter Key ==> Rec. %d[s]", MEM_SIZE/Fs);
                            getchar();                              // [Enter]入力待ち
                            printf("Rec.\n");                       // 録音中の表示
                        }
                        else{
                            printf("Enter Key ==> Play %d[s]", MEM_SIZE/Fs);
                            getchar();                              // [Enter]入力待ち
                            printf("Play\n");                       // 再生中の表示
                        }
                    }
                    if(Rec==-1){                                    // Recフラグが-1なら逆再生出力
                        y[t]=s[MEM_SIZE-t];
                    }
