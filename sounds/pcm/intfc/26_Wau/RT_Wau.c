...
//************************************************************************//
    ...
    long int l=0;                                                   // 変調用時刻管理
    double   x,u0,u1,u2;                                            // 逆ノッチフィルタ用信号
    double   a,r;                                                   // 逆ノッチフィルタ係数
    double   fc, rt, depth;                                         // 変調用の変数
    ...
    r    = 0.95;                                            // 極半径の２乗
    fc   = 1000.0;                                          // 遮断周波数[Hz]
    depth= 800.0;                                           // 変調の深さ[Hz]
    rt   = 1.0;                                             // 変調の周波数
    a    =-(1+r)*cos(2.0*M_PI*fc/Fs);                       // 逆ノッチフィルタ係数
...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                ...
                u0   = s[t] - a*u1 - r*u2;                          // 内部信号
                x    = r*u0 + a*u1 + u2;                            // オールパスフィルタ出力
                y[t] = (s[t]-x)/2.0;                                // 逆ノッチフィルタ出力
                u2   = u1;                                          // 内部信号の遅延
                u1   = u0;                                          // 内部信号の遅延
                fc   = (1000.0 + depth*sin(2.0*M_PI*rt*l/Fs))/Fs;   // 遮断周波数を変化させる
                a    = -(1+r)*cos(2.0*M_PI*fc);                     // フィルタ係数を再設定
                l    = (l+1)%(10*Fs);                               // 変調信号用の時刻更新
