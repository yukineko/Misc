...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    if(hflg==1)y[t]=y[(t-1+MEM_SIZE)%MEM_SIZE];     // 過去の値を保持する場合
                    if(hflg==2)y[t]=0;                              // 間の値を0とする場合
                    if(t%DS==0)y[t]=s[t];                           // 指定回数に1回だけ出力=入力とする
...
//                    y[t]=atan(y[t])/(M_PI/2.0);                         // クリップ防止
        ...
        if (kbhit()){                                               // キーボード入力があったか
            ...
            if (key == 32){                                         // スペースキーが押されとき
                ...
                if(hflg==1)printf("1/%d keep previous value\n",DS); // 過去の値を保持
                if(hflg==2)printf("1/%d zero interpolation \n",DS); // 間を0で埋める
            }
