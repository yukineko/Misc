...
//************************************************************************//
    ...
    double     z[Fs+1]={0}, z1[Fs+1]={0};                                       // 出力信号格納用
    double     zv[Fs+1]={0};                                                    // 垂直エッジ用配列
    int      k;                                                                 // 時刻の変数
...
//                  Xamp[m] = 10*(pow( 10, (B+G+R)/3.0/20.0 )-1);                 // スペクトログラムを常用対数とみなす
            ...
            for(i=0;i<2*hi;i++){                                                // 出力計算
                z1[i]=z[i];                                                     // 1フレーム前の信号を記録
                z[i]=0, zv[i]=0;
                for(k=0;k<hi;k++){
                    z[i] =  z[i] + Xamp[k]/hi * cos(2.0*M_PI*k/(2.0*hi)*i);
                    zv[i]= zv[i] + (Xamp[k+1]-Xamp[k])/hi * cos(2.0*M_PI*k/(2.0*hi)*i);// 垂直方向の差分を振幅とする（垂直エッジ抽出）
                }
            }
            t=0;
            n++;
          ...
          y[t] = z[t]-z1[t]+zv[t];                                              // フレーム間差分（水平エッジ）に垂直エッジを加算
