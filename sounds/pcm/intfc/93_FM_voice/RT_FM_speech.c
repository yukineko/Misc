...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    x_FM = 0.5*cos(2*M_PI*Fc/Fs*t + theta);             // FM変調信号
        
                    xI[k]= x_FM *   cos(2*M_PI*Fc/Fs*t) ;               // 実部の信号
                    xQ[k]= x_FM * (-sin(2*M_PI*Fc/Fs*t));               // 虚部の信号
                    ...
                    phi1  = phi;                                        // 過去の位相
                    if( yQ+yI!=0 && yI*yQ!=0){
                        phi   = atan2(yQ, yI);                          // 現在の位相
                    }
                    y[t]  = phi - phi1;                                 // 復調信号
                    if( fabs(y[t]) > M_PI ){
                        y[t] = y[t] - y[t]/fabs(y[t])*2*M_PI;           // 位相の折り返し対応
                    }
                    if(hflg==1)y[t]=x_FM;                               // FM変調信号
