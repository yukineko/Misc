...
//************************************************************************//
    ...
    double s_L[MEM_SIZE+1]={0}, s_R[MEM_SIZE+1]={0};        // 左右のマイクロホン入力
...
//************************************************************************//
        ...
        x1_L[l] = x_L[FFT_SIZE/2+l]=s_L[t];                 // FFT_SIZE/2以降の入力をx[l]に格納

         x_R[l] = x1_R[l];                                  // FFT_SIZE/2までの入力をx[l]に格納
        x1_R[l] = x_R[FFT_SIZE/2+l]=s_R[t];                 // FFT_SIZE/2以降の入力をx[l]に格納
        ...
        if( l>=FFT_SIZE/2){                                 // 半フレームごとにFFTを実行
            ...
            for(i=0;i<FFT_SIZE;i++){
                G[i]=1.0;                                   // ゲインを1にする
                if(Xamp_L[i] > Xamp[i]) G[i]=0;             // Rchが小さければゲインを0にする
                Xr[i]=G[i]*Xr[i];                           // 実部にゲインをかける
                Xi[i]=G[i]*Xi[i];                           // 虚部にゲインをかける
            }
            ifft();                                         // IFFT
            l=0;
        }
