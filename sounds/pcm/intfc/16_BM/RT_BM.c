...
//************************************************************************//
    ...
    double s_L[MEM_SIZE+1],    s_R[MEM_SIZE+1];                     // 左右のマイクロホン入力
    double x_L[FFT_SIZE+1]={0},x1_L[FFT_SIZE+1]={0};                // LchのFFT用信号
    double x_R[FFT_SIZE+1]={0},x1_R[FFT_SIZE+1]={0};                // RchのFFT用信号
    double Xamp_L[FFT_SIZE+1], G[FFT_SIZE+1]={0};                   // Lchの振幅保存とバイナリマスク
    double BMout;                                                   // 出力信号
...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    if(t%2==1){
                         x_L[l] = x1_L[l];                      // FFT_SIZE/2までの入力をx[l]に格納
                        x1_L[l] = x_L[FFT_SIZE/2+l]=s[t];       // FFT_SIZE/2以降の入力をx[l]に格納
                    }
                    else{
                         x_R[l] = x1_R[l];                      // FFT_SIZE/2までの入力をx[l]に格納
                        x1_R[l] = x_R[FFT_SIZE/2+l]=s[t];       // FFT_SIZE/2以降の入力をx[l]に格納

                        BMout   = (z[l]+z1[l])/FFT_SIZE;        // IFFTで得た出力
                        z1[l]   = z[FFT_SIZE/2+l];
                        l++;
                    }
                    if( l>=FFT_SIZE/2){
                        ...
                        //                       //
                        ///////////////////////////
                        for(i=0;i<FFT_SIZE;i++){                // LchのFFT用信号に窓を掛ける
                            xin[i]=x_L[i]*0.5*(1.0-cos(2.0*M_PI*i/(double)FFT_SIZE));
                        }
                        fft();                                  // LchのFFT
                        for(i=0;i<FFT_SIZE;i++){
                            Xamp_L[i]=Xamp[i];                  // Lchの振幅スペクトル
                        }
                        ///////////////////////////
                        //                       //
                        //     RchのFFT処理      //
                        //                       //
                        ///////////////////////////
                        for(i=0;i<FFT_SIZE;i++){                // RchのFFT用信号に窓を掛ける
                            xin[i]=x_R[i]*0.5*(1.0-cos(2.0*M_PI*i/(double)FFT_SIZE));
                        }
                        fft();                                  // RchのFFT
                        ////////////////////////
                        // バイナリマスキング //
                        ...
                        for(i=0;i<FFT_SIZE;i++){
                            G[i]=1.0;                           // ゲインを1にする
                            if(Xamp_L[i] < Xamp[i]) G[i]=0;     //処理変更フラグが1ならLに指向性
                            if(hflg==2)G[i]=1.0-G[i];           //処理変更フラグが2ならRに指向性
                            Xr[i]=G[i]*Xr[i];
                            Xi[i]=G[i]*Xi[i];
                        }
                        ifft();                                     // IFFT
...
//                    y[t]=atan(y[t])/(M_PI/2.0);                     // クリップ防止
        ...
        if (kbhit()){                                               // キーボード入力があったか
            ...
            if (key == 32){                                         // スペースキーが押されとき
            	hflg=(hflg+1)%3;                                    // 処理フラグ変更
                if(hflg==0) printf("Through\n");                    // 素通しであることを表示
                if(hflg==1) printf("Left Extracted!\n");                 // 処理中であることを表示
                if(hflg==2) printf("Right Extracted!\n");                 // 処理中であることを表示
            }
