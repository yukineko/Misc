...
#include <string.h>
#include <math.h>
#define  MEM_SIZE 16000                                     // 音声信号用メモリサイズ
#define  FFT_SIZE 32                                        // FFT点数
///////////////////////////////////////////////
//                                           //
//                                           //
//            FFT関数の構築                  //
...
//************************************************************************//
    ...
    double    g[FFT_SIZE+1]={0};                            // フィルタリング係数
    double   u0[FFT_SIZE+1]={0}, u1[FFT_SIZE+1]={0};        // オールパスフィルタの遅延信号
    double   a;                                             // オールパスフィルタのフィルタ係数
...
//************************************************************************//
        ...
        for(i=1;i<FFT_SIZE;i++){
            u0[i] =     x[i-1] + a * u1[i];                 // IIR部出力
             x[i] = -a * u0[i] + u1[i];                     // APF出力＝次段への入力
            u1[i] = u0[i];                                  // 信号遅延処理
        }
        ...
        ///////////////////////////
        for(i=0;i<=FFT_SIZE/2;i++){
            Xr[i]=Xr[FFT_SIZE-i]=0.0;                       // ゲインを0とする
            if( i>10 ){                                      // 全部で16帯域
                Xr[i]=Xr[FFT_SIZE-i]=1.0;                   // 指定周波数のゲインを1にする(BPF)
            }
            Xi[i]=Xi[FFT_SIZE-i]=0.0;                       // 虚部は常に0
        }
        ifft();
        for(i=0;i<FFT_SIZE;i++){
            g[i] = z[(FFT_SIZE/2+i)%FFT_SIZE]/FFT_SIZE;     // ゲインのIFFTを巡回シフトしてフィルタ係数をつくる
            g[i] = g[i]*w[i];                               // フィルタ係数に窓を掛ける
        }
        y[t] = 0;                                           // 出力を初期化
        for(i=0;i<FFT_SIZE;i++){
            y[t] = y[t] + x[i]*g[i];                        // 各APF出力をフィルタリング
        }
