...
static double Xamp[FFT_SIZE+1]={0}, Xphs[FFT_SIZE+1]={0};            // 振幅と位相
static double omega;                                                 // FFT角周波数
static int    St,br[FFT_SIZE+1];                                     // FFTステージ番号，ビット反転
static double c[FFT_SIZE+1]={0};                                     // ケプストラム
static double XFin[FFT_SIZE+1]={0};                                  // 対数微細構造
static double XEnv[FFT_SIZE+1]={0};                                  // 対数スペクトル包絡
...
void cep_FE(void);                                          // ケプストラム変換
void icep_FE(void);                                         // ケプストラム逆変換
...
//************************************************************************//
            ...
            ////////////////////////////////////////////////////
                    ...
                    if( l>=FFT_SIZE/2){                             // 半フレームごとにFFTを実行
                        ...
                        ave_env=0;
                        for(i=0;i<FFT_SIZE;i++){
                            ave_env += XEnv[i]/FFT_SIZE;                // スペクトル包絡の平均値計算
                        }
                        for(i=0;i<FFT_SIZE;i++){
                            XEnv[i]  = ave_env;                         // スペクトル包絡を定数にする
                        }
